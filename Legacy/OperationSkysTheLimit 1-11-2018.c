...
// ------------------------------------------------------------------- Lift Control
void liftUser (int height, int speed, bool clawHold = false)
{
	...
	{
		while (SensorValue(LiftPot) < height - 320)
		{
...
void driveLift (int driveSpeed, float distance, int liftSpeed, int height, bool clawHold = false)
{
	...
	{
		while ((abs(SensorValue[LeftEncoder]) < ticks) && (SensorValue[LiftPot] > height + 400)) //80, 20, 80, 1500     || 0 < 20 , 1400 > 1500
		{
			int rDiff = abs(SensorValue[LeftEncoder]) - abs(SensorValue[RightEncoder]);
			float rMod = sgn(rDiff) * driveSpeed * rModMulti;
	...
	{
		while ((abs(SensorValue[LeftEncoder]) < ticks) && (SensorValue[LiftPot] < height - 200)) //80, 20, 80, 1500     || 0 < 20 , 1400 > 1500
		{
			int rDiff = abs(SensorValue[LeftEncoder]) - abs(SensorValue[RightEncoder]);
			float rMod = sgn(rDiff) * driveSpeed * rModMulti;
	...
	//checks if reached DISTANCE first
	...
	//checks if reached HEIGHT first
	if (SensorValue[LiftPot] < height)
	{
		while((abs(SensorValue[LeftEncoder]) < ticks))
		{
...
void highGoalRedRight (int cones)
{
	if (cones == 2)
	{
		...
		lift (1100, 80, true);
		wait1Msec(300);
		driveEncoder (-60, 10 , true);
		wait1Msec(300);
		turnGyro (-70, 93, true);
		wait1Msec(300);
		driveEncoder (60, 13, true);
		wait1Msec(200);
		lift (1700, 80);
		claw (-80, 120);
...
void special ()
{
	...
	wait1Msec(300);
	lift (2800, 70);
	driveEncoder (60, 10.5);
	claw (127, 500);
	//score second cone on high goal
	lift (1100, 80, true);
	wait1Msec(300);
	turnGyro (-70, 135, true);
	driveEncoder (120, 11, true);
	wait1Msec(200);
	lift (1700, 80);
	claw (-80, 120);
...
task autonomous()
{
	...
	default: //Test
		displayLCDCenteredString(0, autonTest); displayLCDCenteredString(1, "Running!"); highGoalBlueRight (); break;
	}
}
...
task usercontrol()
{
...

	{
		...
		//	motor[MobilePush] = -127; SensorValue[LED] = 0;}
		//else if (vexRT[Btn7UXmtr2] == 1)
		//	motor[MobilePush] = 127;
		//else if (vexRT[Btn8UXmtr2] == 1)
		//	motor[MobilePush] = -127;
		//else{
		//	motor[MobilePush] = 0; SensorValue[LED] = 1;}
		...
		if (vexRT[Btn7D] == 1) //Opens Auton Selector
			autoSelector();

		if (vexRT[Btn7U] == 1){ //Shows Battery Voltage
			if (!showBattery){
		...
		if (vexRT[Btn7R] == 1) //Sets to High Goal Red
			myAutonomous = 3;
		if (vexRT[Btn7L] == 1) //Sets to High Goal Blue
			myAutonomous = 4;

		if (!showBattery) //Shows Selected Auton
			displayLCD();
